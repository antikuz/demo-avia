...
func (storage *Storage) List(departure_city string, arrival_city string, dateFrom string, dateTo string) []models.FlightsV {
	query := fmt.Sprintf(`
	SELECT flight_id, scheduled_departure_local, scheduled_arrival_local, scheduled_duration, departure_city, arrival_city FROM flights_v fv
